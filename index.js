...
module.exports = function (ret, conf, settings) {
    ...
    fis.util.map(ret.src, function (subpath, file) {
        ...
        if(path.extname(fileName) === '.html'/*~file.toString().indexOf('center.html')*/){
            ...
            if(iconList.length > 0) {
                var cssContent = icon.generateCss(iconList, iconMaps.iconContentMaps);
    ...
    function getIconFromHtmlAndDepTpl(file, depTpls) {
        ...
        var getMatches = function (content) {
            var matches = content.match(/[\s\'\"]i-([^\s\'\"]*)/g);
            if(matches){
                for(var i=0; i<matches.length; i++){
                    matches[i] = matches[i].replace(/[\s\'\"]i-/g, '');
                }
            }
            return matches;
        };

        matches = getMatches(htmlContent);
        if(matches) {
        ...
        for(var k=0,len=depTpls.length; k<len; k++) {
            ...
            matches = getMatches(tplFileContent);
            if(matches) {
                iconList = iconList.concat(matches);
            }
        }
    ...
    function getIconFromJs(jsDeps) {
        ...
        // 这里的匹配可能会有问题
        var getMatches = function (content) {
            var matches = content.match(/[\s\'\"]i-([a-zA-Z0-9\-_]*[\'\"])/g);
            if(matches){
                for(var i=0; i<matches.length; i++){
                    matches[i] = matches[i].replace(/[\s\'\"]i-/g, '');
                }
        ...
        for(var k=0,len=jsDeps.length; k<len; k++) {
            ...
            matches = getMatches(jsFileContent);
            if(matches) {
                iconList = iconList.concat(matches);
            }
        }
